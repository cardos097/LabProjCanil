<x-app-layout>
    <div class="max-w-6xl mx-auto p-4">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-bold">Adoções</h1>
        ...
        <div class="overflow-x-auto bg-white border rounded">
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="text-left p-3">Animal</th>
                ...
                <tbody>
                    @forelse($adoptions as $adoption)
                        <tr class="border-t">
                            <td class="p-3 font-medium">{{ $adoption->animal->name ?? '—' }}</td>
                            <td class="p-3">{{ $adoption->user->name ?? '—' }}</td>
                            <td class="p-3">{{ $adoption->status }}</td>
                            <td class="p-3">{{ $adoption->created_at?->format('Y-m-d H:i') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-3">Sem pedidos de adoção.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
